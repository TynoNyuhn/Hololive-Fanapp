...
struct InanisNinomae: View {
    ...
    @State var videoName = "IV1"
    @State var videoPlayer = AVPlayer(url: Bundle.main.url(forResource: "IV1", withExtension: "mp4")!)
    @State var videos = ["IC1", "AC5", "IC2", "GC2"]
    @State var videoPosition = 0
    @State var mediaTypes  = ["BGM", "Music Videos", "Random Videos"]
    @State var mediaChoice = 0
    @State var images = ["IV1", "IV2", "GV3"]
    @State var audioPlayer: AVAudioPlayer!
    @State var soundFile = "IBGM2"
    @State var BGMs = ["IBGM", "IBGM2"]
    ...
    @State private var pageName: String = "InanisNinomae"
    
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack {
                Text("Ina'nis Ninomae")
                    .font(.aquire(size:48))
                    .bold()
                ...
                if (mediaChoice != 0) {
                    VideoPlayer(player: videoPlayer)
                        ...
                        .overlay(Rectangle()
                                    .strokeBorder(
                                        LinearGradient(gradient: Gradient(colors: [.orange, .black]), startPoint: .leading, endPoint: .trailing),
                                        lineWidth: 5
                                    )
                        ).onAppear {
                ...
                VStack {
                    ...
                    Text("One day, Ina'nis picked up a strange book and then started to gain the power of controlling tentacles. To her, tentacles are just a part in her ordinary life; it has never been a big deal for her. However, her girly mind does want to get them dressed up and stay pretty.\n\nAfter gaining power, she started hearing Ancient Whispers and Revelations. Hence, she began her VTuber activities to deliver random sanity checks on humanity, as an ordinary girl.")
                        .font(.aquire(size:24))
                        .padding()
                        .foregroundColor(Color.yellow)
                    Text("Ina has a very laidback and calming personality. Often drawing to pass the time.")
                        .font(.aquire(size:24))
                        .padding()
                        .foregroundColor(Color.yellow)
                    Text("Birthday: May 20th\nDebut Date: September 13, 2020\nFanbase: Takodachis")
                        .font(.aquire(size:24))
                        .padding()
                        .foregroundColor(Color.yellow)
                    HStack {
                        VStack {
                        Link(destination: URL(string: "https://www.youtube.com/channel/UCMwGHR0BTZuLsmjY_NT5Pwg")!) {
                            Image("YouTube")
                                .resizable()
                        ...
                        VStack {
                        Link(destination: URL(string: "https://twitter.com/ninomaeinanis")!) {
                            Image("Twitter")
                                .resizable()
...
struct InanisNinomae_Previews: PreviewProvider {
